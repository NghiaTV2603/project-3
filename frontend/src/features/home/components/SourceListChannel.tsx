...
import channelSlice, { Channel, fetchChannel } from '../channelSlice';
import useSettingChannel from '../../../hooks/modal/useSettingChannel';

function SourceListChannel() {
   const navigate = useNavigate();
   const theme = useTheme();
   const colors = tokens(theme.palette.mode);
   const { channels, currentChannel } = useAppSelector(
      (state) => state.channel
   );
   const { modal, handleOpen } = useSettingChannel();
   const { currentServer } = useAppSelector((state) => state.server);
   const dispatch = useAppDispatch();
   useEffect(() => {
      if (currentServer?.id) {
         dispatch(fetchChannel(currentServer.id));
      }
   }, [currentServer]);
   const selectChannel = (channel: Channel) =>
      dispatch(channelSlice.actions.selectChannel(channel));
   return (
      <Stack>
         <Accordion
            defaultExpanded
               ...
               {channels
                  ...
                  .map((e) => (
                     <Stack
                        ...
                        justifyContent="space-between"
                        bgcolor={
                           currentChannel?.id === e.id ? colors.grey[600] : ''
                        }
                        sx={{
                           '&:hover': {
